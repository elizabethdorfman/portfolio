...
try {
  ...
  const skillAssets = assets.filter(name => name.endsWith('.md'));
  const pages = [{ path: '/blog', title: 'Elizabeth\'s Software Engineering Blog', summary: 'Writing by Elizabeth Dorfman about software and AI. Thoughts on designing simple, intuitive technology.' }, ...posts.map(post => ({ ...post, path: `/blog/${post.slug}` }))];
  for (const page of pages) {
    const url = origin + page.path;
    const image = origin + (page.image ?? '/write-like-a-human-preview.png');
    ...
    const metadata = { description: page.summary, 'og:site_name': 'Elizabeth\'s Software Engineering Blog', 'og:title': page.title, 'og:description': page.summary, 'og:type': page.slug ? 'article' : 'website', 'og:url': url, 'og:image': image, 'og:image:alt': page.imageAlt ?? 'Write like a human. Real before-and-after examples and a free AI writing skill by Elizabeth Dorfman.', 'twitter:card': 'summary_large_image', 'twitter:title': page.title, 'twitter:description': page.summary, 'twitter:image': image };
    ...
    let body = render(page.path);
    for (const asset of skillAssets) {
      const source = asset.replace(/-[^-]+\.md$/, '.md');
      body = body.replaceAll(`/src/data/${source}`, `/assets/${asset}`);
    }
